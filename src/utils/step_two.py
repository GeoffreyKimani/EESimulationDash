...
from src.utils.utils import defineROI, calculate_cwsi
...
def get_sentinel_images(aoi, start_date, end_date, cloud_probability_threshold):
    ...
    filtered_s2_collection = s2_collection.filterDate(
        start_date, end_date
    ).filterBounds(aoi)
    filtered_cloud_probability_collection = cloud_probability_collection.filterDate(
        start_date, end_date
    ).filterBounds(aoi)
    ...
    def mask_clouds(s2_image):
        ...
        cloud_probability_image = ee.Image(
            filtered_cloud_probability_collection.filterMetadata(
                "system:index", "equals", s2_image.get("system:index")
            ).first()
        )

        # Create a cloud mask where the cloud probability is less than the threshold.
        cloud_mask = cloud_probability_image.select("probability").lt(
    ...
    for index in expected_indices:
        if index in mean_values:
            ...
            print(
                f"Index {index} not found in the image. Available keys: {mean_values.keys()}"
            )
...
def get_landsat_images(
    ...
    while attempt < max_attempts:
        ...
        if masked_collection.size().getInfo() > 0:
            ...
            cwsi = (
                None  # calculate_cwsi(medianComposite, tw, ts) # compute the CWSI index
            )
    ...
    print(
        f"No suitable images found within {max_attempts} attempts. Returning an empty collection."
    )
    # Return None if no suitable images found
    return None, {
        {"CWSI": None, "EVI": None, "NDVI": None, "SAVI": None, "GNDVI": None}
    }


def get_indices_with_fallback(
    aoi_ee, start_date, end_date, initial_threshold=20, increment=5, max_threshold=50
):
    ...
    while threshold <= max_threshold:
        image, indices_dict = get_sentinel_images(
            aoi_ee, start_date, end_date, cloud_probability_threshold=threshold
        )
...
def integrate_indices_to_dataframe(df, satellite="LANDSAT"):
    for index, row in df.iterrows():
        ...
        if satellite == "LANDSAT":
            ...
            band_img, indices_dict = get_landsat_images(
                aoi_ee, row["start_date"], row["end_date"]
            )
        else:
            band_img, indices_dict = get_indices_with_fallback(
                aoi_ee, row["start_date"], row["end_date"]
            )
